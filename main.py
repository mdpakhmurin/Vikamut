...
for event in longpoll.listen():
    ...
    if event.type == VkBotEventType.MESSAGE_NEW:
        ...
        try:
            ...
            if (config.peer_ids is not None and len(config.peer_ids) != 0 and msg.peer_id not in config.peer_ids):
                continue
            ...
            if filters.is_delete(msg):
                ...
                print(datetime.now(), "\t", f"Message deleted (from: https://vk.com/id{msg.from_id}, peer_id:{msg.peer_id}): {msg.text}")
        except Exception as e:
            print(datetime.now(), "\t", "error filtering message")
